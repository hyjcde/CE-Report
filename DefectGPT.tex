...
\begin{abstract}
Building defect management faces challenges in handling evolving defects across complex urban structures. This paper develops DefectGPT, a framework integrating Unmanned Aerial Vehicle (UAV) inspection, Building Information Modeling (BIM), and Geographic Information Systems (GIS) into a Digital Twin (DT) environment. The framework introduces three innovations: (1) a dynamic knowledge engine that automatically updates defect patterns from multi-source data streams; (2) a natural language interface enabling contextual queries; and (3) an explainable analysis module linking maintenance decisions to historical cases and regulatory standards. To our knowledge, this is the first implementation of Retrieval-Augmented Generation (RAG) in building defect management, where domain-specific knowledge retrieval guides Large Language Models (LLMs) to generate compliance-aware recommendations. A 12-month deployment on a Hong Kong high-rise shows systematic correlation analysis and real-time decision support compared to document-driven approaches. This work provides a paradigm for automated defect diagnosis in infrastructure maintenance workflows, advancing DT applications in urban sustainability.
\end{abstract}
...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{Overall Framework.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{System Class.png}
